...
/*   Updated: 2024/01/12 00:09:42 by lauger           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_image(t_display_info *image_info)
{
	...
	while (i != 5)
	{
		if (image_info->image_array[i].img == NULL)
			close_windows(image_info);
		i++;
	}
	return (0);
}

int	mbx_links(char **map)
{
	void			*mlx;
	void			*mlx_win;
	...
	image_info.map = map;
	if (init_image(&image_info))
		return (-1);
	ft_check_symbol(map, &image_info, image_info.image_array);
	mlx_hook(image_info.mlx_win, 17, 0, close_windows, &image_info);
